...
- Rename the keyword `bracha_toueg` in any foldername (/bracha_toueg and /docs/bracha_toueg), in any filenames (docs/bracha_toueg/bracha_toueg.rst) and in any fields in any file (all instances in index.rst) to a **meaningfull abbreviation** of your distributed algorithm. Note that doc/substitutions.rst defines a keywork |Bracha-Toueg| that has to be changed as well.
...
- Populate the rst files under the docs/bracha_toueg: astract, algorithm, conclusion, introduction, results with the extension rst following restructured text syntax. The templates will guide you on what to write in each file. There is also a rubric for self-assessment.
- List your modules in code.rst under docs/bracha_toueg
